...
while(1)
{
	if(etat == RUN || etat == TERM || etat ==ERREUR || etat == EXIT || acquire_line( fp2,  inter2)  == 0)
	{ 
//		while (etat != PAUSE || etat != NOT )
		{
			switch(etat)
			{
				...
				char * token2 = get_next_token(inter2);
					...
					else if(strcmp(token2,"step")==0) {PC=PC+4;etat = RUN;}
...
definition recherche_dictionnaire(definition dictionnaire_commande,uint32_t word)
{
	...
	while(k<41)
		...
		{
			DEBUG_MSG("NOM dicos : %s",dictionnaire_commande[k].nom);
			strcpy(def_instruction->nom,dictionnaire_commande[k].nom);
			DEBUG_MSG("NOM extrait : %s ",def_instruction->nom);
...
int execut_instruction(map_reg * mrg,mem memoire,instruction inst)
{
	...
	fonction_MIPS = choix_fonction(0); // A remplacer
